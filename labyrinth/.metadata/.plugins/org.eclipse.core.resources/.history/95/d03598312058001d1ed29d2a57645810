...
void Window::create_window() {
	_window = std::shared_ptr<SDL_Window>(
			SDL_CreateWindow("labyrinth Yakovleva", SDL_WINDOWPOS_CENTERED,
...
Window::load_texture(const char *filename) {
	...
	result = std::shared_ptr<SDL_Texture>(
			IMG_LoadTexture(_renderer.get(), filename), SDL_DestroyTexture);
	if (result == nullptr)
		throw std::runtime_error(
...
Window::Window(int w, int h) : _width(w), _height(h) {
	...
	auto [t, tw, th] = load_texture("wall.jpg");
	_wall_tex = t;
	_wall_tex_width = tw;
	_wall_tex_height = th;
...
void Window::draw_minimap(SDL_Renderer *r) {
	...
	SDL_Rect pr { x1 - DOT_SIZE / 2, y1 - DOT_SIZE / 2, DOT_SIZE, DOT_SIZE };
	SDL_SetRenderDrawColor(r, 64, 255, 64, 255);
	SDL_RenderFillRect(r, &pr);
	SDL_SetRenderDrawColor(r, 125, 85, 47, 1);
...
void Window::render(SDL_Renderer *r) {
	...
	SDL_SetRenderDrawColor(r, 57, 8, 102, 1); // potolok
	SDL_RenderFillRect(r, &cr);
	SDL_SetRenderDrawColor(r, 95, 100, 102, 1); // pol
